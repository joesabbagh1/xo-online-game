...
class Network:
    ...
    def connect(self):
        try:
            ...
            print(f"Connection to {self.addr} was refused.")
        except TimeoutError:
            print(f"Connection to {self.addr} timed out.")
...
def main():
    network = Network()
    game = TicTacToeGame()
    board = TicTacToeBoard(game, network)

    # create network object and run it in a separate thread
    t = threading.Thread(target=network.connect)
    t.start()

    t2 = threading.Thread(target=listen, args=(board, network))
    t2.start()
